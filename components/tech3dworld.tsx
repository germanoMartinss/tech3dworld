...
export function Tech3DWorld() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      ...
      <section id="processo" className="border-y border-white/10 bg-card/40 px-5 py-24 lg:px-8 lg:py-32"><div className="mx-auto max-w-7xl xl:max-w-[90rem]"><p className="eyebrow lg:text-xs">03 / como acontece</p><h2 className="mt-4 max-w-2xl font-mono text-4xl font-bold uppercase text-white sm:text-5xl lg:mt-6 lg:max-w-3xl lg:text-6xl">Uma ideia.<br /><span className="text-secondary">Várias dimensões.</span></h2><div className="mt-14 grid gap-8 md:grid-cols-4 lg:mt-20 lg:gap-10">{['Descobrir', 'Projetar', 'Construir', 'Evoluir'].map((step, i) => <div key={step} className="relative border-t border-white/20 pt-5 lg:pt-6"><span className="font-mono text-sm text-primary lg:text-base">0{i + 1}</span><h3 className="mt-8 font-mono text-xl font-bold uppercase text-white lg:mt-10 lg:text-2xl">{step}</h3><p className="mt-3 text-sm leading-6 text-muted-foreground lg:mt-4 lg:text-base lg:leading-7">{['Entendemos o desafio e descobrimos novas possibilidades.', 'Damos forma à visão com precisão e intenção.', 'Prototipamos, testamos e materializamos.', 'O projeto vive, aprende e encontra novos caminhos.'][i]}</p></div>)}</div></div></section>

      <section id="contato" className="relative px-5 py-28 text-center lg:px-8 lg:py-36"><div className="gridlines pointer-events-none absolute inset-0 opacity-30" /><div className="relative mx-auto max-w-4xl lg:max-w-5xl"><Layers3 className="mx-auto size-10 text-primary lg:size-14" /><h2 className="mt-7 text-balance font-mono text-5xl font-black uppercase leading-none tracking-tight text-white sm:text-7xl lg:mt-9 lg:text-8xl">Vamos criar<br /><span className="text-gradient">o próximo mundo.</span></h2><p className="mx-auto mt-7 max-w-xl text-lg leading-8 text-muted-foreground lg:mt-9 lg:max-w-2xl lg:text-xl lg:leading-9">Você traz a pergunta. A gente constrói a possibilidade.</p><a href="mailto:hello@tech3dworld.com" className="mt-9 inline-flex rounded-full bg-secondary px-7 py-4 font-bold text-secondary-foreground transition hover:shadow-[0_0_35px_rgba(0,204,255,0.4)] lg:mt-12 lg:px-9 lg:py-5 lg:text-lg">hello@tech3dworld.com <ArrowRight className="ml-3 size-5 lg:size-6" /></a></div></section>
